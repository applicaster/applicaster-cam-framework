...
typedef enum _APASICachePolicy {
	...
	APASIUseDefaultCachePolicy = 0,

	// Tell the request not to read from the cache
	APASIDoNotReadFromCacheCachePolicy = 1,

	// The the request not to write to the cache
	APASIDoNotWriteToCacheCachePolicy = 2,
	...
	APASIAskServerIfModifiedWhenStaleCachePolicy = 4,

	// Always ask the server if there is an updated version of this resource (using a conditional GET)
	APASIAskServerIfModifiedCachePolicy = 8,

	// If cached data exists, use it even if it is stale. This means requests will not talk to the server unless the resource they are requesting is not in the cache
	APASIOnlyLoadIfNotCachedCachePolicy = 16,

	// If cached data exists, use it even if it is stale. If cached data does not exist, stop (will not set an error on the request)
	APASIDontLoadCachePolicy = 32,

	// Specifies that cached data may be used if the request fails. If cached data is used, the request will succeed without error. Usually used in combination with other options above.
	APASIFallbackToCacheIfLoadFailsCachePolicy = 64
} APASICachePolicy;
...
typedef enum _APASICacheStoragePolicy {
	APASICacheForSessionDurationCacheStoragePolicy = 0,
	APASICachePermanentlyCacheStoragePolicy = 1
} APASICacheStoragePolicy;
...
- (APASICachePolicy)defaultCachePolicy;

- (BOOL)canUseCachedDataForRequest:(APASIHTTPRequest *)request;

// Should Remove cached data for a particular request
- (void)removeCachedDataForRequest:(APASIHTTPRequest *)request;
...
- (BOOL)isCachedDataCurrentForRequest:(APASIHTTPRequest *)request;

// Should store the response for the passed request in the cache
// When a non-zero maxAge is passed, it should be used as the expiry time for the cached response
- (void)storeResponseForRequest:(APASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge;
...
- (NSString *)pathToStoreCachedResponseHeadersForRequest:(APASIHTTPRequest *)request;

// Returns the location to use to store a cached response body for a particular request
- (NSString *)pathToStoreCachedResponseDataForRequest:(APASIHTTPRequest *)request;

// Clear cached data stored for the passed storage policy
- (void)clearCachedResponsesForStoragePolicy:(APASICacheStoragePolicy)cachePolicy;
