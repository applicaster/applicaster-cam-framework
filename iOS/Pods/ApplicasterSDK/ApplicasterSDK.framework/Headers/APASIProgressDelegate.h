...
@protocol APASIProgressDelegate <NSObject>
...
- (void)setAPMaxValue:(double)newMax;
#endif

// Called when the request receives some data - bytes is the length of that data
- (void)request:(APASIHTTPRequest *)request didReceiveBytes:(long long)bytes;
...
- (void)request:(APASIHTTPRequest *)request didSendBytes:(long long)bytes;

// Called when a request needs to change the length of the content to download
- (void)request:(APASIHTTPRequest *)request incrementDownloadSizeBy:(long long)newLength;

// Called when a request needs to change the length of the content to upload
// newLength may be less than zero when a request needs to remove the size of the internal buffer from progress tracking
- (void)request:(APASIHTTPRequest *)request incrementUploadSizeBy:(long long)newLength;
@end
